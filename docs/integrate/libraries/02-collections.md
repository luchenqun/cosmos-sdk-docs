...
Cosmos SDK模块使用`KVStore`接口来处理它们的状态。使用`KVStore`的问题在于，它强制你将状态视为字节KV对，而实际上大部分状态来自于复杂的具体的golang对象（字符串、整数、结构体等）。

集合允许你像处理普通的golang对象一样处理状态，并且在代码中不需要将状态视为原始字节。

它还允许你迁移现有的状态，而不会导致任何状态破坏，从而迫使你进行繁琐和复杂的链状态迁移。

## 安装

要在你的cosmos-sdk链项目中安装集合，请运行以下命令：
...
集合提供了5种不同的API来处理状态，这些API将在下一节中进行探讨，这些API包括：
- ``Map``：用于处理类型化的任意KV对。
- ``KeySet``：仅用于处理类型化的键
- ``Item``：仅用于处理一个类型化的值
- ``Sequence``：一个单调递增的数字。
- ``IndexedMap``：将``Map``和`KeySet`结合起来，提供具有索引功能的`Map`。

## 初始组件

在探索不同的集合类型及其功能之前，有必要介绍每个集合共享的三个组件。实际上，当通过执行例如```collections.NewMap/collections.NewItem/...``` you will find yourself having to pass them some common arguments.
...
func (k Keeper) CreateAccount(ctx sdk.Context, addr sdk.AccAddress, account authtypes.BaseAccount) error {
	...
	if has {
		return fmt.Errorf("账户已存在：%s", addr)
	}
...
func (k Keeper) IterateAccountsBetween(ctx sdk.Context, start, end uint64) ([]authtypes.BaseAccount, error) {
	// collections.Range API提供了很多功能，比如定义迭代的起始和结束位置。
